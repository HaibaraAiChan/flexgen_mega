...
shape2= 8
input_tensor = torch.tensor([[[ 0.7140,  1.2694,  1.9161, -0.3093, -0.4700, -0.0953, -0.3145,
          -0.3397],
...
output_whole = F.layer_norm(input_tensor, input_tensor[2].shape)
print('output_whole ', output_whole[:,0:1,:])

# Applying LayerNorm individually to each (4, 1, 768) slice along dimension 2
slices = torch.chunk(input_tensor, input_tensor.size(1), dim=1)
print('slices[0].shape ', slices[0].shape)
# print('slices ', slices)
layer_norm_slices = [F.layer_norm(slice, slice[2].shape) for slice in slices]
output_slices = torch.cat([layer_norm for layer_norm in layer_norm_slices], dim=1)
