...
class DynamicColumnParallelLinear(nn.Module):
    ...
    def forward(self, x):
        ...
        # y_gathered = [torch.empty_like(y_local) for _ in range(self.num_gpus)]
        # dist.all_gather(y_gathered, y_local)

        # # Stack the gathered results
        # y = torch.stack(y_gathered, dim=0) 
        return y
...
# Compare output and loaded_output for equality
are_equal = torch.equal(output, loaded_output)

print('equal ', are_equal)
print(output[:10])
print(loaded_output[:10])
